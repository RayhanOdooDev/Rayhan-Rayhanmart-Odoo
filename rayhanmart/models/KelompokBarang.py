...
class KelompokBarang(models.Model):
    _name = 'rayhanmart.kelompokbarang'
    ...
    barang_ids = fields.One2many(comodel_name='rayhanmart.barang', 
    ...
    def _compute_jml_item(self):
        for rec in self:
            a = self.env['rayhanmart.barang'].search([('kelompokbarang_id','=', rec.id)]).mapped('name')
            b = len(a)
            rec.jml_item = b
            rec.daftar = a
